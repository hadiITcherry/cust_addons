...
class FSMOrder(models.Model):
    ...
    location_id = fields.Many2one(
        "fsm.location", string="Location", index=True
    )
    ...
    image_1 = fields.Image(string="Image1",attachment=False)
    image_2 = fields.Image(string="Image2",attachment=False)
    image_3 = fields.Image(string="Image3",attachment=False)
    image_4 = fields.Image(string="Image4",attachment=False)

    c_images = fields.Image(string="Technician Images",attachment=False)

    fsm_product_id = fields.One2many("fsm.order.products","fsm_order_id")
    fsm_situation_id = fields.One2many("fsm.order.situation","fsm_order_id")

    t_image_1 = fields.Binary(string='Image 1',attachment=False)
    t_image_2 = fields.Binary(string='Image 2',attachment=False)
    t_image_3 = fields.Binary(string='Image 3',attachment=False)
    t_image_4 = fields.Binary(string='Image 4',attachment=False)
    t_image_5 = fields.Binary(string='Image 5',attachment=False)

    imei = fields.Char(string="IMEI")
    serial_nb = fields.Char(string="Serial Nb")
    note_taking = fields.Text(string="Notes")

    audio = fields.Binary("Audio",attachment=False)
    audio_file_name = fields.Char()

      

    delivery_type = fields.Selection([('dtd','Door to Door'),('drc','Drop it')],string="Delivery Type")
    ...
    service_type = fields.Many2one("fsm.order.type","type")

    # internal_type = fields.Selection(related="fsm.order.type.internal_type")
    ...
    def action_complete(self):
        address_id = self.partner_id.id if self.delivery_type == "dtd" else 321
        ...
        products = self.env['fsm.order.products'].sudo().search([('fsm_order_id','=',self.id)])
        for product in products:
            if product.chosen_product == True:
                self.env['sale.order.line'].with_user(user.id).create({
                    "order_id":order_id,
                    "product_id":product.product_id.id,
                    "name":product.product_id.product_tmpl_id.name,
                    "price_unit":product.product_unit_price,
        ...
        encoded_body = json.dumps({"read":False,"time":int(round(datetime.now().timestamp())),"service_nb":self.name,"message":"Delivery Order Created","status":"Completed","email":self.partner_id.email})
        encoded_body2 = json.dumps({"read":False,"order_nb":order.name,"time":int(round(datetime.now().timestamp())),"status":"Order Received","email":self.partner_id.email})
        partner = http.request.env['res.partner'].sudo().search([('parent_id.id','!=',False)])
        partners = []
        user = None
        for i in partner:
            partners.append(i.id)
        if self.partner_id.id in partners:
            current_id = self.partner_id.parent_id.id
            user = self.env['res.users'].sudo().search([('partner_id.id','=',current_id)])
        else:
            user = self.env['res.users'].sudo().search([('partner_id.id','=',self.partner_id.id)])
        http_request.request("PUT", "https://quico-tech-default-rtdb.europe-west1.firebasedatabase.app/Services/"+str(user.id)+"/"+str(self.id)+".json",body=encoded_body,headers={"content-type":"application/json","auth":"AIzaSyDaoLknXHqhJY_zcKlAxV2UX0tnl0OdM5w"})
        http_request.request("PUT", "https://quico-tech-default-rtdb.europe-west1.firebasedatabase.app/Orders/"+str(user.id)+"/"+str(order_id)+".json",body=encoded_body2,headers={"content-type":"application/json","auth":"AIzaSyDaoLknXHqhJY_zcKlAxV2UX0tnl0OdM5w"})
        return self.write(
            {
    ...
    def action_cancel(self):
        http_request = None
        http_request = urllib3.PoolManager()
        encoded_body = json.dumps({"read":False,"time":int(round(datetime.now().timestamp())),"service_nb":self.name,"status":"Cancelled","email":self.partner_id.email})
        partner = http.request.env['res.partner'].sudo().search([('parent_id.id','!=',False)])
        partners = []
        user = None
        for i in partner:
    ...
    def action_wait(self):
        http_request = None
        http_request = urllib3.PoolManager()
        products = self.env['fsm.order.products'].sudo().search([('fsm_order_id.id','=',self.id)])
    ...
    def action_progress(self):
        http_request = None
        http_request = urllib3.PoolManager()
        encoded_body = json.dumps({"read":False,"time":int(round(datetime.now().timestamp())),"service_nb":self.name,"status":"In Progress","email":self.partner_id.email})
        partner = http.request.env['res.partner'].sudo().search([('parent_id.id','!=',False)])
        partners = []
        user = None
        for i in partner:
...
class FsmOrderProducts(models.Model):
    _name = "fsm.order.products"

    fsm_order_id = fields.Many2one("fsm.order")
    product_id = fields.Many2one('product.product',string="Product")
    product_uom = fields.Many2one(string="Uom",related='product_id.product_tmpl_id.uom_id',store=True,readonly=True)
    product_unit_price = fields.Float(string="Unit Price",related='product_id.product_tmpl_id.standard_price',store=True,readonly=True)
    quantity = fields.Float(string="quantity")
    subtotal = fields.Float(string="Subtotal",compute="calculate_subtotal",store=True)
    chosen_product = fields.Boolean(default=False,string="Chosen")

    @api.depends('quantity')
    def calculate_subtotal(self):
        for rec in self:
            subtotal = rec.product_unit_price * rec.quantity
        self.subtotal = subtotal

class FsmSituations(models.Model):
    _name= "fsm.situations"

    name = fields.Char(string="Name")

class FsmOrderSituation(models.Model):
    _name = "fsm.order.situation"

    fsm_situation_id = fields.Many2one("fsm.situations",string="Name")
    fsm_order_id = fields.Many2one("fsm.order")
